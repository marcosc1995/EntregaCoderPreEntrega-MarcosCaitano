...
const router = Router();
//mongodb
const ProductsContainerFile = require(`../../dao/daoMongoProducts`);
//firebase
//const ProductsContainerFile = require(`../../dao/firebaseProductsDAO`);
...
router.delete("/api/productos/:id", async (req, res) => {
  if (admin) {
    try {
      ...
      const elementDeleted = await productsContainerAccess.deleteById(
        idRecieved
      );
...
router.put("/api/productos/:id", async (req, res) => {
  if (admin) {
    try {
      ...
      const product = {
        ...req.body,
        timestamp: `Modificado: ${new Date().toLocaleString()}`,
      };
      const productUpdated = await productsContainerAccess.modifyProductById(
        idRecieved,
        product
      );
